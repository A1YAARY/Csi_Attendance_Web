...
const Dashbord2 = ({ dashboard }) => {
    ...
    const [lastactivaty, setLastactivaty] = useState(dashboard.latestActivities || []);
                ...
                <div className="bg-white min-h-[30rem] w-full flex flex-col items-center justify-start rounded-2xl mt-4 sm:mt-6 p-6 sm:p-8 gap-6 border border-gray-200 shadow-md">
                    ...
                    <div className="w-full flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-bold text-gray-800">Latest Activities</h2>
                        </div>
                        ...
                        <div className="h-full overflow-y-auto pr-2 [scrollbar-width:thin] [scrollbar-color:#a8a29e_transparent]">
                            {lastactivaty.length > 0 ? (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    {/* Activity Icon */}
                                                    <div className={`p-2 rounded-full ${activity.type === 'check-in' ? 'bg-green-100' : 'bg-red-100'}`}>
                                                ...
                                                <div className="text-right">
                                                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${activity.type === 'check-in' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                        <div className={`h-2 w-2 rounded-full ${activity.type === 'check-in' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                                        {activity.type === 'check-in' ? 'Check In' : 'Check Out'}
                                                    </div>
                                                    <div className="text-gray-600 text-sm mt-1 font-medium">
                                                        {/* Using the pre-formatted timeIST field from your data */}
                                                        {activity.timeIST ? new Date(activity.timeIST).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : new Date(activity.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-200">
                                                <div className="flex items-center gap-2">
                                                    <div className={`h-2 w-2 rounded-full ${activity.verified ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                                                    <span className="text-xs text-gray-500 font-medium">
                                                        {activity.verified ? 'Verified' : 'Pending'}
                                                    </span>
                                                </div>
                                                <span className="text-xs text-gray-500">
                                                    {/* Using the pre-formatted timeIST field for the date */}
                                                    {activity.timeIST ? new Date(activity.timeIST).toLocaleDateString('en-IN') : new Date(activity.time).toLocaleDateString('en-IN')}
                                                </span>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <h3 className="text-lg font-medium text-gray-600 mb-1">No activities yet</h3>
                                    <p className="text-sm text-center">Recent check-ins and check-outs will appear here.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="w-full grid grid-cols-3 gap-4 pt-4 border-t border-gray-200">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-gray-800">{lastactivaty.filter(a => a.type === 'check-in').length}</div>
                            <div className="text-xs text-gray-500 font-medium">Today's Check-ins</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-gray-800">{lastactivaty.filter(a => a.type === 'check-out').length}</div>
                            <div className="text-xs text-gray-500 font-medium">Today's Check-outs</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-gray-800">{lastactivaty.length}</div>
