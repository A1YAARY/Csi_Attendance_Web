...
const bulkRegisterUsers = handleAsync(async (req, res) => {
  if (!req.file) {
  ...
  if (orgContext.error) {
    throw new ApiError(400, orgContext.error);
  }
  ...
  if (jsonData.length > 1000) {
    throw new ApiError(400, "Too many users. Please process in batches of 1000 or fewer.");
  }
...
const downloadTemplate = handleAsync(async (req, res) => {
  const templateData = [
    {
