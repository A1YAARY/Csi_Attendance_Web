...
exports.scanQRCode = handleAsync(async (req, res) => {
  const userOrgId = (
  ...
  if (!code) {
    throw new ApiError(400, "Missing required field: code");
  }
  ...
  if (!qr) {
    ...
    if (!qr) {
      throw new ApiError(404, "QR code not found or inactive");
    }

    if (qr.organizationId?.toString() !== userOrgId) {
      throw new ApiError(403, "QR code belongs to another organization");
    }
  }
...
exports.getUserPastAttendance = handleAsync(async (req, res) => {
...
exports.getDailyReport = handleAsync(async (req, res) => {
...
exports.getWeeklyReport = handleAsync(async (req, res) => {
...
exports.checkWorkingDay = handleAsync(async (req, res) => {
  const { date } = req.query;
  if (!date) {
